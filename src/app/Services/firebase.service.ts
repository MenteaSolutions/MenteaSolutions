...
export class FirebaseService {
  ...
  async changeUserPassword(newPassword: string, userDetails: any): Promise<void> {
    ...
    if (currentUser) {
      try {
        ...
        await this.updateUserPassword(userDetails.roomName, userDetails.uid, newPassword);
        console.log('Password updated successfully in Firebase Realtime Database.');
  ...
  async updateUserPassword(roomName: string, userId: string, newPassword: string): Promise<void> {
    try {
      const userRef = ref(this.db, `rooms/${roomName}/users/${userId}`);
      await update(userRef, { password: newPassword }); // Mise à jour du mot de passe
      console.log('Password updated successfully in Firebase Realtime Database.');
    } catch (error) {
      console.error('Error updating password in Firebase Realtime Database:', error);
      throw error;
    }
  }
}